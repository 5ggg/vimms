...
class TopNController(Controller):
    def __init__(self, mass_spec, ionisation_mode, N, isolation_window, mz_tol, rt_tol, min_ms1_intensity):
        super().__init__()
    ...
    def _process_scan(self, scan):
        ...
        if self.last_ms1_scan is not None:
            ...
            for i in idx:
                ...
                dda_scan_params = self.get_dda_scan_param(mz, intensity, self.isolation_window,
    ...
    def get_dda_scan_param(self, mz, intensity, isolation_window, mz_tol, rt_tol):
...
class HybridController(TopNController):
    def __init__(self, mass_spec, ionisation_mode, N, scan_param_changepoints,
                 isolation_window, mz_tol, rt_tol, min_ms1_intensity,
                 n_purity_scans=None, purity_shift=None, purity_threshold=0):
        super().__init__(mass_spec, ionisation_mode, N, isolation_window, mz_tol, rt_tol, min_ms1_intensity)
        self.mass_spec = mass_spec
        self.ionisation_mode = ionisation_mode
        self.N = np.array(N)
            ...
            for i in idx:
                ...
                if purity < self.purity_threshold:
                    ...
                    for purity_idx in range(self.n_purity_scans):
                        # create a new ms2 scan parameter to be sent to the mass spec
                        dda_scan_params = self.get_dda_scan_param(mz + purity_shift_amounts[purity_idx], intensity, current_isolation_window,
                    ...
                    # create a new ms2 scan parameter to be sent to the mass spec
                    dda_scan_params = self.get_dda_scan_param(mz, intensity, current_isolation_window,
                                                              current_mz_tol, current_rt_tol)
                    new_tasks.append(dda_scan_params)
                    fragmented_count += 1
